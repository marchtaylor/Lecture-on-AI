...
layer_outputs <- lapply(model$layers[2:7], function(layer) layer$output) 
...
grid1 <- expand.grid(lon = , lat = , lag = -3:-1)
...
grid <- expand.grid(lon = seq(30,135,5), lat = seq(-67.5,27.5,5), channel = 1:num_convf1)
grid$activation <- as.vector(first_layer_activation)

grid1 <- expand.grid(lon = seq(30,135,5), lat = seq(-67.5,27.5,5))
...
grid <- expand.grid(lon = seq(30,135,10), lat = seq(-67.5,27.5,10), channel = 1:num_convf2)
...
grid <- expand.grid(lon = seq(30,135,20), lat = seq(-67.5,27.5,20), channel = 1:num_convf2)
